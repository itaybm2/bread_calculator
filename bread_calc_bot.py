...
from telegram.ext import (Updater, CommandHandler, CallbackQueryHandler, InlineQueryHandler, ConversationHandler, MessageHandler,Filters)
# Enable logging
...
CALCULATOR_SUBMENU,SELECTING_PARAMETER, TYPING, CALCULATOR, CALCULATE, CALCULATED = map(chr, range(7, 13))

# Meta states
GO_BACK,GO_BACK_CALCULATOR,BACK,CALCULATING,SAVE_INPUT,RESET = map(chr,range(14,20))
...
def ask_for_input(update,context):
	...
	update.callback_query.edit_message_text(ask_for_input_message(update.callback_query.data))
	return SAVE_INPUT
...
def save_input(update, context):
	...
	print(curr_input)
	if(curr_input <= 0):
		context.bot.send_message(chat_id=update.message.chat_id,text=invalid_input_message())
		ud[START_OVER] = True
		return calculator_submenu(update, context)
	if(curr_param != "DOUGH_WEIGHT"):
		if(curr_input not in range(1,101)):
			context.bot.send_message(chat_id=update.message.chat_id,text=invalid_input_message())
			ud[START_OVER] = True
			return calculator_submenu(update, context)
...
def calculate(update, context):
	...
	or not ud[FEATURES].get("SD_HYDRATION"):
		print("Here")
		#query = update.callback_query
		context.bot.send_message(chat_id=update.message.chat_id,text="נא ודא שהזנת את כל הפרמטרים.")
		ud[START_OVER] = True
		return calculator_submenu(update, context)
	...
	Water_Per = Final_Hydration * (1 + Salt_Per + Starter_Solid_Per) - Starter_Water_Per
	Flour = Dough_Weight / (1+ Water_Per + Starter_Per + Salt_Per)
	Starter = Starter_Per * Flour
	Salt = Salt_Per * Flour
	Water =  Water_Per * Flour
	print("Starter hydrartion: {} , Final Hydration: {}".format(Starter_Hydration,Final_Hydration))
	print("Bread with total flour of {}g, {}g Water, {}g Starter, {}g Salt".format(Flour,Water,Starter,Salt))
	calc_text = "קמח: {:.1f} גרם\n מים: {:.1f} גרם\n מחמצת: {:.1f} גרם\n מלח: {:.1f} גרם\n".format(Flour,Water,Starter,Salt)
...
def calculator_submenu_keyboard():
	keyboard = [[InlineKeyboardButton("משקל בצק", callback_data="DOUGH_WEIGHT")],
					[InlineKeyboardButton("הידרציה סופית", callback_data="HYDRATION")],
					[InlineKeyboardButton("מחמצת", callback_data="STARTER")],
					[InlineKeyboardButton("הידרציית מחמצת", callback_data="SD_HYDRATION")],
...
def main():
	...
	updater.bot.setWebhook('https://telegrambreadbot.herokuapp.com/' + TOKEN)
	print("HERE_MAIN")
	...
	calculator_conv_handler = ConversationHandler(
		...
		states={
		CALCULATOR_SUBMENU:[
		CallbackQueryHandler(ask_for_input, pattern='(DOUGH_WEIGHT|HYDRATION|STARTER|SALT|SD_HYDRATION)'),
		CallbackQueryHandler(calculate, pattern='^' + str(CALCULATE) + '$'),
		CallbackQueryHandler(reset_data, pattern='^' + str(RESET) + '$')
		],
		SAVE_INPUT: [
